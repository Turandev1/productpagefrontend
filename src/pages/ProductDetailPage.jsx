...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        // API { success, product } qaytarır — product-ı çıxar
        setProduct(data.product || data)
      } catch (err) {
        setError(err.message)
        toast.error('Məhsul yüklənərkən xəta baş verdi: ' + err.message)
      } finally {
        setLoading(false)
      }
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="mt-4 text-sm sm:text-base text-gray-500">Məhsul yüklənir...</p>
      </div>
    )
  }
  ...
  const formattedDate = new Date(product.createdAt).toLocaleDateString('az-AZ', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  })
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
          ...
          <div className="p-6 sm:p-8 lg:p-10 flex flex-col justify-center">
            ...
            <div className="prose prose-gray max-w-none">
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Məhsul Açıqlaması</h3>
      ...
      {showWhatsAppConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 sm:p-8 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">WhatsApp-a yönləndirilsin?</h3>
            <p className="text-sm text-gray-500 mb-6">
              {product.phoneNumber} nömrəsi ilə WhatsApp üzərindən əlaqə saxlamaq istədiyinizə əminsiniz?
            </p>
