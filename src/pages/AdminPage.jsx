...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!cancelled) toast.error('Ürünler yüklenemedi: ' + err.message)
      } finally {
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      ...
      // Token hatası durumunda oturumu temizle
      if (err.message.includes('token') || err.message.includes('Yetkilendirme')) {
        handleLogout()
        return
      }
  ...
  // Token var ama oturum doğrulanmamışsa şifre formu göster
  if (!sessionVerified) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <SessionPasswordForm
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Ürünleri yönetin ve yeni ürünler ekleyin</p>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 sm:p-8">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                {editFiles.length > 0 && (
                  <p className="mt-1 text-xs text-gray-400">{editFiles.length} yeni görsel seçildi</p>
                )}
              </div>
